...
object Schedulers {
  ...
  def demoActorWithTimeout(): Unit = {
    val timeoutActor: Behavior[String] =
      Behaviors
        .receive {
          (context, message) =>
   ...
   *
   */
  private object ResettingTimeoutActor {
    def apply(): Behavior[String] =
      Behaviors
        .receive {
          (context, command) =>
            context.log.info(s"Received Input Command : $command")
            resettingTimeoutActor(context.scheduleOnce(1.second, context.self, "timeout"))
        }

    //behaviour with reset-ing the schedular
    private def resettingTimeoutActor(schedule: Cancellable): Behavior[String] =
      Behaviors
        .receive {
          (context, message) =>
            message match {
