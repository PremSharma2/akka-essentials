...
class UsingProbesSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A master actor" should {
    ...
    "register a worker" in {
      ...
      /**
       * a TestProbe is not a “mock actor” like Mockito.
       * In Akka Typed, a TestProbe[T] is a real actor (a little mailbox + behavior)
       * that records every message it receives,
       * so you can later assert on that mailbox
       * with expectMessage, receiveMessage, etc.
       */
    ...
    "aggregate data correctly" in {
      ...
      val mockedWorkerBehavior =
        Behaviors
        .receiveMessage[WorkerTask] {
...
object UsingProbesSpec {
  ...
  case class Work(text: String, replyTo: ActorRef[ExternalProtocol]) extends MasterCommand

  case class WorkCompleted(count: Int, originalDestination: ActorRef[ExternalProtocol]) extends MasterCommand

  case class Register(workerRef: ActorRef[WorkerTask], replyTo: ActorRef[ExternalProtocol]) extends MasterCommand

  //Worker Commands

  case class WorkerTask(text: String, master: ActorRef[MasterCommand], originalDestination: ActorRef[ExternalProtocol])
  ...
  object Master {
    def apply(): Behavior[MasterCommand] = Behaviors.receiveMessage {
