...
class InterceptingLogsSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A checkout flow" should {
    "correctly dispatch an order for a valid credit card" in {
      LoggingTestKit
        .info("Order") // filter log messages of level INFO which contain the string "Order"
...
object InterceptingLogsSpec {
  ...
  case class Checkout(item: String, creditCard: String) extends PaymentProtocol // paymentCommand

  case class AuthorizeCard(creditCard: String, replyTo: ActorRef[PaymentProtocol]) extends PaymentProtocol // paymentCommand

  case object PaymentAccepted extends PaymentProtocol // paymentCommand

  case object PaymentDeclined extends PaymentProtocol // paymentCommand

  case class DispatchOrder(item: String, replyTo: ActorRef[PaymentProtocol]) extends PaymentProtocol // paymentCommand

  case object OrderConfirmed extends PaymentProtocol // paymentCommand

  object CheckoutActor {
    def apply(): Behavior[PaymentProtocol] =
      Behaviors.setup {
        context =>
