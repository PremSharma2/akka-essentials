...
class TimedAssertionsSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A worker actor" should {
    ...
    val probe = testKit.createTestProbe[OutPutEvent]()
...
object TimedAssertionsSpec {
  trait OutPutEvent
  case class WorkResult(result: Int) extends OutPutEvent

  trait InputCommand
  case class Work(replyTo: ActorRef[OutPutEvent]) extends InputCommand
  case class WorkSequence(replyTo: ActorRef[OutPutEvent]) extends InputCommand

  object WorkerActor {
    def apply(): Behavior[InputCommand] = Behaviors.receiveMessage {
      case Work(replyTo) =>
